...
function fixture(
  options: { temporary?: boolean; releaseFails?: boolean } = {},
) {
...
describe("supervisor identity runtime", () => {
  it("does not start harness before room established; pending retries only observation", async () => {
    ...
    const room = {
      ...
      observe: async () =>
        established ? ("established" as const) : ("pending" as const),
      discardSecret: async () => {},
    };
    try {
      await expect(
      ...
      await expect(f.runtime.prepare(room, 500)).rejects.toThrow(
        "ROOM_PENDING",
      );
...
it("controller generation change cannot replay an uncertain room invite", async () => {
  ...
  let redeems = 0;
  try {
    const c = await RuntimeController.acquire(
      lockRoot,
      "daemon",
      "Agent",
      "instance",
...
it("pinned CID and action cannot change on an existing runtime", async () => {
  ...
  try {
    ...
    expect(
      () =>
        new AgentOursRuntime({ ...f.assignment, expectedCid: "OTHER" }, f.deps),
    ).toThrow("MISMATCH");
    expect(
      () => new AgentOursRuntime({ ...f.assignment, action: "new" }, f.deps),
    ).toThrow("MISMATCH");
  } finally {
    f.cleanup();
  }
