...
export type Exec = (cmd: string, args: string[], opts?: { env?: NodeJS.ProcessEnv; timeout?: number }) => Promise<ExecResult>;

/** execFile wrapper that never rejects; missing binary → code 127. */
export const realExec: Exec = (cmd, args, opts) =>
  new Promise(resolve => {
    execFile(cmd, args, { env: opts?.env ?? process.env, timeout: opts?.timeout, maxBuffer: 10 * 1024 * 1024 },
      (err, stdout, stderr) => {
        let code = 0;
        if (err) {
