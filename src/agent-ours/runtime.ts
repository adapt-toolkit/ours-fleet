...
export class AgentOursRuntime {
  ...
  async prepare(room?: RoomAdmission, timeoutMs = 30_000): Promise<void> {
    if (
      ['TERMINAL_INTENT', 'CLEANUP_PENDING', 'RELEASED', 'FAILED', 'QUIESCING'].includes(
        this.state.phase,
      )
    )
    ...
    if (!this.state.cid) {
      ...
      if (this.state.phase === 'RECOVERING') throw Error('UNCERTAIN_PROVISIONING');
      ...
      if (row) {
        ...
        const created =
          this.state.lifetime === 'temporary'
            ...
            : await this.deps.client.createIdentity({ ...args, requireExistingRoot: true });
        if (created.hierarchy !== 'role') throw Error('ROOT_PROVISIONING_FORBIDDEN');
