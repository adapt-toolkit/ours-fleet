...
export async function prepareManagedAgent(
  ...
  const launch = temporary
    ? JSON.parse(
        readFileSync(
          join(root, "launches", binderKey("temporary", role.name) + ".json"),
          "utf8",
        ),
      )
    : undefined;
  ...
  if (existsSync(instancePath)) {
    ...
    try {
      writeFileSync(
        instancePath,
        JSON.stringify({ instance, role: role.name, temporary }),
        {
          flag: "wx",
          mode: 0o600,
        },
  ...
  try {
    ...
    if (prior?.phase === "RELEASED") {
      ...
      atomicPrivateWrite(instancePath, {
        instance,
        role: role.name,
        temporary,
      });
    ...
    const client = await attachOursClient({
      ...
      requiredCapabilities: [
        "external-sessions-v1",
        "root-first-identities-v1",
      ],
    });
      ...
      room = {
        ...
        observe: async (agentCid) => {
          ...
          if (!observed || observed.identity_cid !== startup.room_identity_cid)
            return "mismatch";
          const seat = observed.seats.find(
            (s) => s.invite_id === startup.invite_id,
          );
          if (
            seat &&
            (seat.identity_cid !== agentCid || seat.role !== startup.role)
          )
            return "mismatch";
          return seat?.seat_state === "active" &&
            contacts.contacts.some(
              (c) => c.container_id === startup.room_identity_cid,
            )
            ? "established"
            : "pending";
        },
    ...
    if (role.roomMemberStartup) {
      ...
      atomicPrivateWrite(
        join(
          root,
          "room-inputs",
          binderKey(startup.room_identity_cid, role.name) + ".ready.json",
        ),
        {
          room: startup.room_id,
          invite: startup.invite_id,
...
export async function preparePermanentAssignment(
  ...
  try {
    ...
    const dir = join(
      privateRuntimeRoot(),
      binderKey(profile.expectedInstanceId, role.identity),
    );
    mkdirSync(dir, { recursive: true, mode: 0o700 });
...
export function readRoomReadiness(
  ...
  | { room: string; invite: string; cid: string; generation: number }
  | undefined {
  try {
    return JSON.parse(
      readFileSync(
        join(
          privateRuntimeRoot(),
          "room-inputs",
          binderKey(roomCid, name) + ".ready.json",
        ),
        "utf8",
      ),
    );
