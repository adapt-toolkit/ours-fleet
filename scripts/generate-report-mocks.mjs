...
const reports = [
  base('overview', 'Fleet overview', 'A safe cross-resource operational snapshot.', [
    ...
    table('agents','Agents',['Role','Harness','Readiness'],[row('developer',V('Role','Developer'),V('Harness','Codex'),V('Readiness','Ready','good'))]),
  ...
  base('task','Task details','Full safe task record and terminal state.',[
    ...
    table('members','Members',['Name','Role','State'],[row('developer',V('Name','developer'),V('Role','Developer'),V('State','Working','good')),row('critic',V('Name','critic'),V('Role','Critic'),V('State','Reviewing','warning'))]),
  ...
  base('room','Room details','Lifecycle, linkage, seats, and safe orchestration readiness.',[details('room','Room',[V('ID','rm-demo'),V('Name','HTML reports'),V('State','Active','good'),V('Task','tsk-demo'),V('Template','pair-review'),V('Saga','members-ready'),V('Health','Ready','good')]),table('seats','Seats',['CID','Role','Seat state','Briefing'],[row('seat-1',V('CID','A1B2…'),V('Role','Developer'),V('Seat state','verified','good'),V('Briefing','delivered','good')),row('seat-2',V('CID','C3D4…'),V('Role','Critic'),V('Seat state','verified','good'),V('Briefing','acknowledged','good'))])]),
  base('room-members','Room members','Participant roles and readiness.',[table('members','Members',['Identity','Role','Seat','Readiness'],[row('member-a',V('Identity','A1B2…'),V('Role','Owner'),V('Seat','authenticated','good'),V('Readiness','Ready','good')),row('member-b',V('Identity','C3D4…'),V('Role','Developer'),V('Seat','verified','good'),V('Readiness','Working','good'))])]),
  base('agents','Fleet agents','Safe session and process readiness.',[table('agents','Agents',['Role','Harness','Lifecycle','Readiness','Activity','Restarts'],[row('developer',V('Role','Developer'),V('Harness','Codex'),V('Lifecycle','Running','good'),V('Readiness','Ready','good'),V('Activity','2026-08-28T09:58:00.000Z'),V('Restarts',0)),row('watcher',V('Role','Watcher'),V('Harness','ACP'),V('Lifecycle','Degraded','warning'),V('Readiness','Recovering','warning'),V('Activity','2026-08-28T09:42:00.000Z'),V('Restarts',2,'warning'))])]),
  base('agent-status','Agent status','Allowlisted lifecycle and readiness; no console, prompt, log, or session secret.',[details('agent','Developer',[V('Role','Developer'),V('Harness','Codex'),V('Lifecycle','Running','good'),V('Ready',true,'good'),V('Model','gpt-5'),V('Activity','2026-08-28T09:58:00.000Z'),V('Restart policy','resume'),V('Isolation','enabled','good')])]),
  ...
  base('template','Template details','Contract, room policy, and member slots.',[details('template','pair-review',[V('Name','pair-review'),V('Version','2'),V('Content hash','sha256:0123…'),V('Anonymous',false),V('Quiet membership',true)] ,[ 'Developer implements; Critic independently reviews every material decision.' ]),table('members','Member slots',['Slot','Count','Role','Reference'],[row('developer',V('Slot','developer'),V('Count',1),V('Role','Developer'),V('Reference','roles.developer')),row('critic',V('Slot','critic'),V('Count',1),V('Role','Critic'),V('Reference','roles.critic'))])]),
